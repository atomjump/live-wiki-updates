<?php
	 ...
	 foreach($argv as $entry) {
	 	if($entry == "email") $set_email = true;
	 	if($entry == "notifications") $set_notifications = true;
	 }
	 ...
	 if(is_null($set_notifications)) {
	 	echo "\n\nDo you want to configure your notifications settings now? [Y/n]:\n";
		$input = rtrim(fgets(STDIN));
		if(($input == "n")||($input == "N")) {
			//Skipping
			echo "\nSkipping notification configuration.\n\n";
	 ...
	 if(is_null($set_email)) {
	
		echo "\nDo you want to configure your email settings now? [Y/n]: (note: you should have an SMTP server account ready. You can configure this later.)\n";
		$input = rtrim(fgets(STDIN));
		if(($input == "n")||($input == "N")) {
			//Skipping
	...
	if($set_notifications == true) {
		...
		if($config_json) {
			...
			$config_json->atomjumpNotifications->notifyAdminWhenLoadAbove = 80;
	...
	if($set_email == true) {
		...
		if($config_json) {
			...
			file_put_contents($config_file, json_encode($config_json, JSON_PRETTY_PRINT|JSON_UNESCAPED_UNICODE|JSON_UNESCAPED_SLASHES));
			echo "Updated main Appliance email configuration.\n";
